#!/usr/bin/env python3
"""
ChronoTracker — واجهة التسعير المحلية (Local Web App)
=====================================================
خادم بسيط يفتح واجهة جميلة بالمتصفّح لتسعير الساعات.
يشتغل فوق نفس محرك التسعير (watch_engine.py).

التشغيل:
  cd ~/Desktop && python3 pricing_app.py
ثم افتح المتصفّح على:  http://localhost:8000

يحتاج: chronotracker_complete_v2.csv و watch_engine.py بنفس المجلد.
"""
...
IMAGES_DIR = os.path.join(_APP_DIR, 'images')
FAVORITES_FILE = os.path.join(_APP_DIR, 'favorites.json')
_CSV = os.path.join(_APP_DIR, 'chronotracker_complete_v2.csv')
_DISC = os.path.join(_APP_DIR, 'discontinued_rolex.csv')

print("جاري تحميل المحرك والبيانات ...")
ENGINE = WatchValuationEngine(csv_path=_CSV, discontinued_csv=_DISC)

# فهرس الموديلات للبحث (autocomplete)
_df = ENGINE.sold
import pandas as _pd
_yr = _pd.to_numeric(_df['year'], errors='coerce')
...
# سنوات كل موديل (كل السنوات الموجودة فعلاً، مع استبعاد المستحيلة للموديلات المتوقفة)
_years_by_ref = {}
...
# معلومات سريعة لكل reference (للعرض في المفضّلة): العلامة + الموديل
_REF_INFO = {}
for _m in SEARCH_INDEX:
    _REF_INFO[_m['ref']] = _m['label']

def fav_list_detailed():
    """قائمة المفضّلة مع الصورة والاسم."""
    out = []
    for ref in load_favorites():
        out.append({
            'ref': ref,
            'label': _REF_INFO.get(ref, ref),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_image(self, path):
        ...
        # اسم الملف بعد /images/ (مرمّز URL، نفك الترميز)
        fname = unquote(path[len('/images/'):])
        # أمان: نمنع الخروج من المجلد
    ...
    def do_GET(self):
        ...
        if u.path == '/' or u.path == '/index.html':
            ...
            self._serve_image(u.path)
        elif u.path == '/api/search':
            q = parse_qs(u.query).get('q', [''])[0]
            results = search_models(q)
            favset = set(load_favorites())
            # نعلّم كل نتيجة لو رفرنسها بالمفضّلة
            results = [dict(m, is_fav=(m['ref'] in favset)) for m in results]
            ...
            try:
                ...
                year = None
            try:
                r = ENGINE.evaluate(reference=ref, year=year, condition=cond,
                                    full_set=fs)
                # نضيف اسم ملف الصورة (لو موجودة)
                ...
                self._send(200, json.dumps({'ok': False, 'msg': str(e)},
                                           ensure_ascii=False))
        elif u.path == '/favorites' or u.path == '/favorites.html':
...
def open_browser():
    ...
    # على ماك: أمر open يشتغل دايماً (حتى من داخل .app)
    try:
        ...
        pass
    # احتياط: webbrowser لأنظمة ثانية
    try:
        webbrowser.open(url)
    except Exception:
        pass
...
if __name__ == '__main__':
    print(f"\n🌐 الواجهة تشتغل على:  http://localhost:{PORT}")
    print("   (بيفتح المتصفّح تلقائياً — أو افتح الرابط بنفسك)")
    ...
    try:
        ...
        # المنفذ مستخدم — التطبيق شغّال أصلاً، نفتح المتصفّح بس
        if 'Address already in use' in str(e) or getattr(e, 'errno', None) in (48, 98):
            print(f"\n⚠️  التطبيق شغّال أصلاً. نفتح المتصفّح على {f'http://localhost:{PORT}'}")
            try:
                import subprocess
