...
def _index_factor_at(brand, ym):
    """معامل التقويم الزمني لشهر ym — المنطق في fair_then.index_factor_at (مشترك مع البوت)."""
    return fair_then.index_factor_at(ENGINE, brand, ym)


# كاش (ref, cond, fs) → السعر المقترح «اليوم» — نفس القاموس المشترك في fair_then
# (يُغذّيه enrich_fair_then من نتيجة الصفحة لتوفير استدعاء evaluate).
_FAIR_TODAY = fair_then.FAIR_TODAY


def _fair_today(ref, cond, fs):
    return fair_then.fair_today(ENGINE, ref, cond, fs)


def _fair_then_pct(ref, brand, cond, fs, price, date_str):
    """شارة «مقابل العادل وقتها» لصف بيع واحد — الحساب نفسه في fair_then.fair_then_pct
    (وحدة مشتركة مع بوت تلقرام حتى تتطابق النسب). يرجّع (fair_then, fair_pct) أو (None, None)."""
    return fair_then.fair_then_pct(ENGINE, ref, brand, cond, fs, price, date_str)
...
function render(d, yearRows){
  ...
  // شارة «مقابل العادل وقتها»: تقارن سعر البيع بالعادل المُرجَّع زمنياً لشهر البيعة.
  // للمباع فقط، ولو توفّر fair_pct من الخادم (وإلا لا شارة بدل رقم مضلّل).
  function fairBadge(s){
    if(!s.sold || s.fair_pct==null) return '';
  ...
  function listingRow(s, withYear){
    return `<tr>
        ...
        <td class="price" style="color:${s.sold?'#5dcaa5':'#f5a3a3'};font-weight:700">${fmt(s.price)}${fairBadge(s)}</td>
    ...
    <a class="dcard" href="/?ref=${encodeURIComponent(d.ref)}">
      ...
      <div class="dbody">
        ...
        <div class="dmeta">
          ...
          <span>${d.cond}</span>
          <span>${d.fs}</span>
          <span>📆 ${d.year?('صنع '+d.year):'سنة غير معروفة'}</span>
        </div>
      </div>
      <div class="dside">
        ...
        if path == '/' or path == '/index.html':
            ...
            try:
                ...
                if r.get('ok') and r.get('reference'):
                    ...
                    if img:
                        r['image_file'] = img
                    enrich_fair_then(r)        # شارة «مقابل العادل وقتها» لصفوف البيع
