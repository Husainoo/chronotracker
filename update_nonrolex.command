#!/bin/bash
# ============================================================
# ChronoTracker — تحديث غير-Rolex (تزايدي) بضغطة واحدة
# يحدّث مزادات كل الماركات ماعدا Rolex تزايدياً (يوقف عند المعروف)، ثم يرفع
...
echo "════════════════════════════════════════════"
echo "  ChronoTracker — تحديث غير-Rolex (تزايدي)"
echo "  المجلد: $REPO_DIR"
echo "════════════════════════════════════════════"
echo ""

# (0) حارس التزامن: زامن مع GitHub قبل أي عمل حتى لا يتعارض جهازان على نفس الريبو
echo "▶ (0) مزامنة مع GitHub (git pull --ff-only) ..."
if ! git pull --ff-only; then
  echo "⚠️  تعذّرت المزامنة (git pull --ff-only) — قد يكون فيه تباعد/تعارض مع الريبو."
  echo "   أوقفنا التشغيل لتفادي التعارض. زامن يدوياً ثم أعد المحاولة. (لم يُسحب أي شيء)"
  echo ""
...
echo "▶ (1/3) جاري تحديث مزادات غير-Rolex (تزايدي) ..."
echo ""
